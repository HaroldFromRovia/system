package ru.itis.kpfu.gametheory;
...
//                0.15*((1/70.9844)*70.9789 - 1) +0.6*((1/0.006662074143595)*0.006663434096883 - 1)));
    ...
    public void makeMatrix() {
        ...
        List<Double> laplas = new ArrayList<>();
        List<Double> vald = new ArrayList<>();
        ...
        for (int k = 0; k < 4; k++) {// перебираем стратегии
            ...
            bias.add(ss0 * cp0 + ss1 * cp1 + ss2 * cp2 + ss3 * cp3);
            var incomes = Arrays.asList(ss0, ss1, ss2, ss3);
            laplas.add(incomes.stream().max(Double::compareTo).get());
            vald.add(incomes.stream().max(Double::compareTo).get());
            pes.add(incomes.stream().min(Double::compareTo).get());
        }

        System.out.println("Laplas :" + laplas);
    ...
    public void initData() {
        ...
        try {
            List<String> pairEUR_RUB = Files.readAllLines(Paths.get("data/GBPRUB_TOD_210901_211130.csv"));
            List<String> pairJPY_RUB = Files.readAllLines(Paths.get("data/EUR_RUB__TOD_210901_211128.csv"));
            List<String> pairUSD_RUB = Files.readAllLines(Paths.get("data/JPYRUB_TOD_210901_211129.csv"));
            List<String> pairUZS_RUB = Files.readAllLines(Paths.get("data/USD000000TOD_210901_211128.csv"));
